...
#' @export
re <- function(df, effect = NULL, se = NULL, use_reml = TRUE) {
  if (is.null(effect)) {
    effect <- df$effect
  }
  if (is.null(se)) {
...
#' @export
fishers_z <- function(df, use_reml = TRUE) {
  meta <- unique(df$meta)
  dof_ <- dof_or_sample_size(df)
  ...
  if (sum(na_rows) > 0) {
    message(paste("Identified", sum(na_rows), "missing DoF values when calculating Fisher's z for meta-analysis", meta))
    df <- df[!na_rows, ]
    dof_ <- dof_or_sample_size(df)
  }
  ...
  re_list <- re(df = re_data, use_reml = use_reml)
